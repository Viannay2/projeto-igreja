...
from models_db import MembroDB, VisitanteDB, UsuarioDB, PresencaDB, LiderancaDB, AnotacaoDB

app = FastAPI(title="API - Sistema de Gestão da Igreja")

# ==========================================================
# CORS — permite que o navegador (rodando o HTML separado)
# consiga chamar essa API.
# ==========================================================
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # em produção, trocar "*" pelo domínio real do site
    allow_methods=["*"],
    allow_headers=["*"],
)

# Cria as tabelas no banco (só na primeira vez — se já existirem, não faz nada)
Base.metadata.create_all(bind=engine)


# ==========================================================
# Modelos Pydantic — validam o formato dos dados que entram/saem
# da API. Diferente dos modelos em models_db.py, que descrevem a
# tabela de verdade no banco.
# ==========================================================
...
class LiderancaEntrada(BaseModel):
    usuario_id: int
    ministerio_nome: str
    papel: str = "lider"  # "lider", "vice" ou "auxiliar"
...
# ==========================================================
# Conversores: linha do banco -> objeto que a API devolve
# ==========================================================
...
# ==========================================================
# Dados iniciais — só entram no banco na PRIMEIRA vez que a
# API roda (se as tabelas já tiverem dado, não mexe em nada)
# ==========================================================
def popular_dados_iniciais():
    ...
    if db.query(MembroDB).count() == 0:
        db.add_all([
            MembroDB(nome_completo="João Silva", familia="Silva", status="ativo", telefone="(21)99999-9999"),
            MembroDB(nome_completo="Maria Sousa", familia="Souza", status="congregando", telefone="(21)98888-8888"),
            MembroDB(nome_completo="Carlos Oliveira", familia="Oliveira", status="afastado", telefone="(21)97777-7777"),
        ])

    if db.query(VisitanteDB).count() == 0:
        db.add_all([
            VisitanteDB(nome="Renata Farias", data_visita="2026-08-03", convidado_por="Maria Sousa", status="ativo"),
            VisitanteDB(nome="Diego Martins", data_visita="2026-07-27", convidado_por="João Silva", status="congregando"),
            VisitanteDB(nome="Beatriz Nogueira", data_visita="2026-07-20", convidado_por="Carlos Oliveira", status="afastado"),
        ])
...
@app.get("/")
def raiz():
    return {"mensagem": "API do Sistema de Gestão da Igreja está no ar"}
...
def listar_membros(db: Session = Depends(get_db)):
    ...
    return [membro_para_saida(m) for m in membros]


@app.post("/api/membros", response_model=Membro)
def criar_membro(membro: Membro, db: Session = Depends(get_db)):
    novo = MembroDB(
...
def excluir_membro(membro_id: int, db: Session = Depends(get_db)):
    membro = db.query(MembroDB).filter(MembroDB.id == membro_id).first()
...
@app.get("/api/membros/{membro_id}", response_model=Membro)
...
@app.put("/api/membros/{membro_id}", response_model=Membro)
def editar_membro(membro_id: int, dados: Membro, db: Session = Depends(get_db)):
    membro = db.query(MembroDB).filter(MembroDB.id == membro_id).first()
    ...
    # data_cadastro NUNCA é alterada aqui de propósito — é fixa desde a criação
...
def listar_visitantes(db: Session = Depends(get_db)):
    return [visitante_para_saida(v) for v in db.query(VisitanteDB).all()]


@app.post("/api/visitantes", response_model=Visitante)
def criar_visitante(visitante: Visitante, db: Session = Depends(get_db)):
    novo = VisitanteDB(
        nome=visitante.nome,
...
def excluir_visitante(visitante_id: int, db: Session = Depends(get_db)):
    visitante = db.query(VisitanteDB).filter(VisitanteDB.id == visitante_id).first()
...
@app.get("/api/visitantes/{visitante_id}", response_model=Visitante)
def buscar_visitante(visitante_id: int, db: Session = Depends(get_db)):
    visitante = db.query(VisitanteDB).filter(VisitanteDB.id == visitante_id).first()
...
@app.put("/api/visitantes/{visitante_id}", response_model=Visitante)
def editar_visitante(visitante_id: int, dados: Visitante, db: Session = Depends(get_db)):
    visitante = db.query(VisitanteDB).filter(VisitanteDB.id == visitante_id).first()
...
@app.post("/api/usuarios", response_model=UsuarioSaida)
def criar_usuario(usuario: UsuarioCadastro, db: Session = Depends(get_db)):
    ...
    return UsuarioSaida(id=novo.id, nome_completo=novo.nome_completo, email=novo.email, cargo=novo.cargo)


@app.post("/api/login", response_model=UsuarioSaida)
def fazer_login(dados: LoginEntrada, db: Session = Depends(get_db)):
    usuario = db.query(UsuarioDB).filter(UsuarioDB.email == dados.email).first()

    senha_confere = usuario and bcrypt.checkpw(dados.senha.encode("utf-8"), usuario.senha_hash)

    if not senha_confere:
        raise HTTPException(status_code=401, detail="E-mail ou senha incorretos")

    return UsuarioSaida(id=usuario.id, nome_completo=usuario.nome_completo, email=usuario.email, cargo=usuario.cargo)


# ----- Rotas de presença/chamada -----

@app.post("/api/chamada", response_model=list[RegistroPresenca])
def salvar_chamada(chamada: ChamadaEntrada, db: Session = Depends(get_db)):
    # Remove chamada anterior desse mesmo ministério nesse mesmo dia, se existir
    ...
    for item in chamada.registros:
        registro = PresencaDB(
            ...
            falta_justificada=item.get("falta_justificada", False),
            motivo_falta=item.get("motivo_falta"),
        )
        db.add(registro)
    ...
    return [presenca_para_saida(r) for r in novos]


@app.get("/api/presencas", response_model=list[RegistroPresenca])
def listar_presencas(db: Session = Depends(get_db)):
    return [presenca_para_saida(p) for p in db.query(PresencaDB).all()]


# ----- Rota de usuários (pro admin escolher quem vira líder) -----

@app.get("/api/usuarios", response_model=list[UsuarioSaida])
def listar_usuarios(db: Session = Depends(get_db)):
    usuarios = db.query(UsuarioDB).all()
    return [
        UsuarioSaida(id=u.id, nome_completo=u.nome_completo, email=u.email, cargo=u.cargo)
        for u in usuarios
    ]


# ----- Rotas de liderança (usuário ↔ ministério ↔ papel) -----

@app.get("/api/liderancas", response_model=list[Lideranca])
def listar_liderancas(db: Session = Depends(get_db)):
    liderancas = db.query(LiderancaDB).all()
    return [
        Lideranca(id=l.id, usuario_id=l.usuario_id, usuario_nome=l.usuario_nome,
                   ministerio_nome=l.ministerio_nome, papel=l.papel)
        for l in liderancas
    ]


@app.post("/api/liderancas", response_model=Lideranca)
def atribuir_lideranca(entrada: LiderancaEntrada, db: Session = Depends(get_db)):
    usuario = db.query(UsuarioDB).filter(UsuarioDB.id == entrada.usuario_id).first()
    ...
    if ja_existe:
        raise HTTPException(status_code=400, detail="Essa pessoa já tem papel de liderança nesse ministério")
    ...
    return Lideranca(id=nova.id, usuario_id=nova.usuario_id, usuario_nome=nova.usuario_nome,
                       ministerio_nome=nova.ministerio_nome, papel=nova.papel)


@app.delete("/api/liderancas/{lideranca_id}")
def remover_lideranca(lideranca_id: int, db: Session = Depends(get_db)):
    lideranca = db.query(LiderancaDB).filter(LiderancaDB.id == lideranca_id).first()
    ...
    db.delete(lideranca)
    db.commit()
    return {"mensagem": "Liderança removida com sucesso"}


# ----- Rotas de anotações do calendário -----

@app.get("/api/anotacoes", response_model=list[Anotacao])
def listar_anotacoes(db: Session = Depends(get_db)):
    anotacoes = db.query(AnotacaoDB).all()
    return [
        Anotacao(id=a.id, data=a.data, ministerio=a.ministerio, texto=a.texto, autor_nome=a.autor_nome)
        for a in anotacoes
    ]


@app.post("/api/anotacoes", response_model=Anotacao)
def criar_anotacao(anotacao: Anotacao, db: Session = Depends(get_db)):
    nova = AnotacaoDB(
        data=anotacao.data,
    ...
    db.commit()
    db.refresh(nova)
    return Anotacao(id=nova.id, data=nova.data, ministerio=nova.ministerio, texto=nova.texto, autor_nome=nova.autor_nome)


@app.delete("/api/anotacoes/{anotacao_id}")
def excluir_anotacao(anotacao_id: int, db: Session = Depends(get_db)):
    anotacao = db.query(AnotacaoDB).filter(AnotacaoDB.id == anotacao_id).first()
    ...
    db.delete(anotacao)
    db.commit()
    return {"mensagem": "Anotação excluída com sucesso"}