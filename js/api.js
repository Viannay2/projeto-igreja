...
function getTokenAuth() {
  return localStorage.getItem("sistema_igreja_token");
}


// Monta os headers automaticamente
function headersComAuth(extraHeaders = {}) {
...
async function getMembros() {
  try {
    ...
    if (!resposta.ok) {
      throw new Error(
        "A API respondeu com erro: " + resposta.status
      );
    }
...
async function addMembro(membro) {
  ...
  if (!resposta.ok) {
    throw new Error(
      "A API não aceitou o cadastro: " + resposta.status
    );
  }
...
async function buscarMembro(id) {
  ...
  if (!resposta.ok) {
    throw new Error(
      "Membro não encontrado: " + resposta.status
    );
  }
...
async function editarMembro(id, membro) {
  ...
  if (!resposta.ok) {
    throw new Error(
      "A API não aceitou a edição: " + resposta.status
    );
  }
...
async function buscarVisitante(id) {
  ...
  if (!resposta.ok) {
    throw new Error(
      "Visitante não encontrado: " + resposta.status
    );
  }
...
async function salvarChamada(data, ministerio, registros) {
  ...
  if (!resposta.ok) {
    throw new Error(
      "A API não aceitou a chamada: " + resposta.status
    );
  }
...
async function getPresencas() {
  ...
  if (!resposta.ok) {
    throw new Error(
      "Não consegui buscar as presenças: " +
      resposta.status
    );
  }
...
async function listarUsuarios() {
  ...
  if (!resposta.ok) {
    throw new Error(
      "Não consegui buscar os usuários: " +
      resposta.status
    );
  }
...
async function getLiderancas() {
  ...
  if (!resposta.ok) {
    throw new Error(
      "Não consegui buscar as lideranças: " +
      resposta.status
    );
  }
...
async function atribuirLideranca(dados) {
  ...
  if (!resposta.ok) {
    ...
    throw new Error(
      erro.detail ||
      "Não foi possível atribuir a liderança"
    );
  }
...
async function getAnotacoes() {
  ...
  if (!resposta.ok) {
    throw new Error(
      "Não consegui buscar as anotações: " +
      resposta.status
    );
  }
